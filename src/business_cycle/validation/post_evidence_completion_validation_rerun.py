"""Phase 36R post-evidence-completion validation rerun bundle."""
...
RUN_ID = "phase36r_post_evidence_completion_validation_rerun_v1"


@lru_cache(maxsize=1)
def build_post_evidence_completion_validation_rerun() -> dict[str, Any]:
    completion = build_recession_recovery_evidence_completion()
    ready = (
        completion["recession_recovery_evidence_completion_runtime_ready"] is True
        and completion["post_predicted_run"]["predicted_label_artifact_count"] == 5
        and completion["post_comparison_run"]["label_comparison_artifact_count"] == 5
        and completion["post_metric_run"]["historical_accuracy_metric_count"] == 5
        and completion["post_trace_run"]["scenario_trace_count"] == 5
        and completion["post_diagnostics_run"]["blocked_scenario_count"] == 0
        and completion["post_diagnostics_run"]["comparable_scenario_count"]
        == completion["post_comparable_scenario_count"]
        and completion["post_result_run"]["historical_validation_result_artifact_count"]
        == 1
        and completion["post_metric_run"]["economic_performance_metric_count"] == 0
        and completion["post_metric_run"]["candidate_phase_emitted"] is False
        and completion["post_metric_run"]["current_phase_emitted"] is False
    )
    return {
        "phase": "36R",
        "run_id": RUN_ID,
        "post_evidence_completion_validation_rerun_ready": ready,
        "updated_research_decision_output_count": completion["post_research_run"][
            "research_decision_output_count"
        ],
        "updated_predicted_label_artifact_count": completion["post_predicted_run"][
            "predicted_label_artifact_count"
        ],
        "updated_comparison_artifact_count": completion["post_comparison_run"][
            "label_comparison_artifact_count"
        ],
        "updated_historical_accuracy_metric_count": completion["post_metric_run"][
            "historical_accuracy_metric_count"
        ],
        "updated_blockage_diagnostic_scenario_count": completion[
            "post_diagnostics_run"
        ]["blockage_diagnostic_scenario_count"],
        "updated_scenario_trace_count": completion["post_trace_run"][
            "scenario_trace_count"
        ],
        "historical_validation_result_artifact_count": completion[
            "post_result_run"
        ]["historical_validation_result_artifact_count"],
        "pre_comparable_scenario_count": completion["pre_comparable_scenario_count"],
        "post_comparable_scenario_count": completion["post_comparable_scenario_count"],
        "false_comparability_count": completion["false_comparability_count"],
        ...
        "research_run": completion["post_research_run"],
        "predicted_run": completion["post_predicted_run"],
        "comparison_run": completion["post_comparison_run"],
        "metric_run": completion["post_metric_run"],
        "trace_run": completion["post_trace_run"],
        "diagnostics_run": completion["post_diagnostics_run"],
        "result_run": completion["post_result_run"],
    }


def summarize_post_evidence_completion_validation_rerun() -> dict[str, Any]:
...
def write_post_evidence_completion_validation_rerun(
    rerun: dict[str, Any],
    *,
    output_dir: str | Path,
) -> dict[str, Any]:
    ...
    completion_path = root / "phase36r_evidence_completion.json"
    research_dir = root / "phase36r_research_decision_outputs"
    predicted_path = root / "phase36r_predicted_label_artifacts.json"
    comparison_path = root / "phase36r_predicted_label_comparison_artifacts.json"
    metric_path = root / "phase36r_historical_accuracy_metrics.json"
    trace_path = root / "phase36r_scenario_validation_trace.json"
    diagnostics_path = root / "phase36r_blockage_diagnostics.json"
    result_path = root / "phase36r_historical_validation_results.json"
    explorer_path = root / "phase36r_research_artifact_explorer.html"
    summary_path = root / "phase36r_post_completion_summary.json"

    written: list[str] = []
    write_result = write_recession_recovery_evidence_completion(
        rerun["completion_run"],
    ...
    result_path.write_text(
        json.dumps(
            {
                "run_id": rerun["result_run"]["run_id"],
                "phase": rerun["result_run"]["phase"],
                "historical_validation_result_runtime_ready": rerun[
                    "result_run"
                ]["historical_validation_result_runtime_ready"],
                "historical_validation_result_artifact": rerun["result_run"][
                    "historical_validation_result_artifact"
                ],
                "scenario_count": rerun["result_run"]["scenario_count"],
                "comparable_scenario_count": rerun["result_run"][
                    "comparable_scenario_count"
                ],
                "non_comparable_scenario_count": rerun["result_run"][
                    "non_comparable_scenario_count"
                ],
                "historical_validation_result_artifact_count": rerun[
                    "result_run"
                ]["historical_validation_result_artifact_count"],
                "historical_accuracy_metric_count": rerun["result_run"][
                    "historical_accuracy_metric_count"
                ],
                "new_accuracy_metric_computed_count": 0,
    ...
    return {
        "output_dir": str(root),
        "post_evidence_completion_validation_rerun_written": True,
...
def _validated_output_dir(output_dir: str | Path) -> Path:
    ...
    if not resolved.is_relative_to(ALLOWED_OUTPUT_ROOT.resolve()):
        raise ValueError(
            f"Phase 36R post-completion output must be under /tmp: {output_dir}"
        )
