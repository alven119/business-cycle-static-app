"""Phase 40 alpha37 controlled current-data refresh freeze."""
...
DEFAULT_ALPHA37_FREEZE_PATH = Path(
    "specs/audits/book_faithful_shadow_v2_alpha37_current_data_refresh_freeze.yaml"
)
PARENT_ALPHA36_FREEZE_PATH = Path(
    "specs/audits/book_faithful_shadow_v2_alpha36_current_research_snapshot_freeze.yaml"
)


@lru_cache(maxsize=1)
def summarize_shadow_current_data_refresh_freeze(
    path: str | Path = DEFAULT_ALPHA37_FREEZE_PATH,
) -> dict[str, Any]:
    freeze = yaml.safe_load(Path(path).read_text(encoding="utf-8"))[
        "book_faithful_shadow_v2_alpha37_current_data_refresh_freeze"
    ]
    ...
    parent = _lightweight_alpha36_parent_summary()
    audit = summarize_phase40_current_data_refresh()
    alpha36_parent_preserved = (
        PARENT_ALPHA36_FREEZE_PATH.exists()
        and parent["freeze_id"] == freeze["parent_freeze_id"]
        and parent["alpha36_freeze_hash_valid"] is True
    )
    ready = (
        not missing
        and not secret
        and not production
        and alpha36_parent_preserved
        and parent["qa12_freeze_unchanged"] is True
        and audit["result"] == "passed"
        and freeze["current_data_refresh_contract_ready"] is True
        and freeze["current_data_refresh_runtime_ready"] is True
        and freeze["current_snapshot_refresh_integration_ready"] is True
        and freeze["current_dashboard_refresh_panel_ready"] is True
        and freeze["ci_hermetic_refresh_tests_ready"] is True
    ...
    return {
        "phase": "40",
        "current_data_refresh_freeze_ready": ready,
        "freeze_id": freeze["freeze_id"],
        "parent_freeze_id": freeze["parent_freeze_id"],
        "freeze_type": freeze["freeze_type"],
        "freeze_manifest_hash": freeze_hash,
        "alpha37_freeze_hash_valid": not missing,
        "freeze_hash_valid": not missing,
        "alpha36_parent_preserved": alpha36_parent_preserved,
        "parent_freeze_present": PARENT_ALPHA36_FREEZE_PATH.exists(),
...
def _lightweight_alpha36_parent_summary() -> dict[str, Any]:
    freeze = yaml.safe_load(PARENT_ALPHA36_FREEZE_PATH.read_text(encoding="utf-8"))[
    ...
    return {
        ...
        "qa12_freeze_unchanged": freeze["qa12_freeze_unchanged"] is True,
        "missing_file_count": len(missing),
    }
