"""Phase 18 alpha14 historical input readiness freeze validation."""
...
DEFAULT_ALPHA14_FREEZE_PATH = Path(
    "specs/audits/book_faithful_shadow_v2_alpha14_historical_input_readiness_freeze.yaml"
)
PARENT_ALPHA13_FREEZE_PATH = Path(
    "specs/audits/book_faithful_shadow_v2_alpha13_historical_manifest_freeze.yaml"
)


def summarize_shadow_historical_input_readiness_freeze(
    path: str | Path = DEFAULT_ALPHA14_FREEZE_PATH,
) -> dict[str, Any]:
    freeze = yaml.safe_load(Path(path).read_text(encoding="utf-8"))[
        "book_faithful_shadow_v2_alpha14_historical_input_readiness_freeze"
    ]
    ...
    parent = summarize_shadow_historical_manifest_freeze()
    readiness = summarize_historical_validation_input_readiness()
    alpha13_parent_preserved = (
        PARENT_ALPHA13_FREEZE_PATH.exists()
        and parent["freeze_id"] == freeze["parent_freeze_id"]
        and parent["historical_manifest_freeze_ready"] is True
    )
    ready = (
        not missing
        and not secret
        and not production
        and alpha13_parent_preserved
        and parent["qa12_freeze_unchanged"] is True
        and readiness["historical_validation_input_readiness_contract_ready"] is True
        and readiness["scenario_input_requirements_ready"] is True
        and readiness["input_readiness_registry_ready"] is True
        and readiness["point_in_time_input_availability_ready"] is True
        and freeze["input_readiness_audited"] is True
