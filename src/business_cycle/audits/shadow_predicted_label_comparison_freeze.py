"""Phase 28 alpha24 predicted-label comparison freeze validation."""
...
DEFAULT_ALPHA24_FREEZE_PATH = Path(
    "specs/audits/book_faithful_shadow_v2_alpha24_predicted_label_comparison_freeze.yaml"
)
PARENT_ALPHA23_FREEZE_PATH = Path(
    "specs/audits/book_faithful_shadow_v2_alpha23_predicted_label_artifact_freeze.yaml"
)


def summarize_shadow_predicted_label_comparison_freeze(
    path: str | Path = DEFAULT_ALPHA24_FREEZE_PATH,
) -> dict[str, Any]:
    freeze = yaml.safe_load(Path(path).read_text(encoding="utf-8"))[
        "book_faithful_shadow_v2_alpha24_predicted_label_comparison_freeze"
    ]
    ...
    parent = summarize_shadow_predicted_label_artifact_freeze()
    readiness = summarize_predicted_label_comparison_readiness()
    alpha23_parent_preserved = (
        PARENT_ALPHA23_FREEZE_PATH.exists()
        and parent["freeze_id"] == freeze["parent_freeze_id"]
        and parent["predicted_label_artifact_freeze_ready"] is True
    )
    ready = (
        not missing
        and not secret
        and not production
        and alpha23_parent_preserved
        and parent["qa12_freeze_unchanged"] is True
        and readiness["predicted_label_comparison_readiness_ready"] is True
        and freeze["predicted_label_comparison_artifact_contract_ready"] is True
        and freeze["predicted_label_comparison_generator_ready"] is True
        and freeze["predicted_label_comparison_readiness_ready"] is True
        and freeze["scenario_count"] == 5
        and freeze["predicted_label_artifact_count"] == 5
        and freeze["label_comparison_artifact_count"] == 5
        and freeze["label_comparison_executed"] is True
        and freeze["predicted_label_provenance_verified_count"] == 5
        and freeze["historical_label_provenance_verified_count"] == 5
    ...
    return {
        ...
        "predicted_label_provenance_verified_count": freeze[
            "predicted_label_provenance_verified_count"
        ],
        "historical_label_provenance_verified_count": freeze[
