"""Phase 21 alpha17 metric preregistration freeze validation."""
...
DEFAULT_ALPHA17_FREEZE_PATH = Path(
    "specs/audits/book_faithful_shadow_v2_alpha17_metric_preregistration_freeze.yaml"
)
PARENT_ALPHA16_FREEZE_PATH = Path(
    "specs/audits/book_faithful_shadow_v2_alpha16_historical_dry_run_freeze.yaml"
)


def summarize_shadow_metric_preregistration_freeze(
    path: str | Path = DEFAULT_ALPHA17_FREEZE_PATH,
) -> dict[str, Any]:
    freeze = yaml.safe_load(Path(path).read_text(encoding="utf-8"))[
        "book_faithful_shadow_v2_alpha17_metric_preregistration_freeze"
    ]
    ...
    parent = summarize_shadow_historical_dry_run_freeze()
    readiness = summarize_historical_metric_readiness()
    alpha16_parent_preserved = (
        PARENT_ALPHA16_FREEZE_PATH.exists()
        and parent["freeze_id"] == freeze["parent_freeze_id"]
        and parent["historical_dry_run_freeze_ready"] is True
    )
    ready = (
        not missing
        and not secret
        and not production
        and alpha16_parent_preserved
        and parent["qa12_freeze_unchanged"] is True
        and readiness["metric_readiness_ready"] is True
        and freeze["historical_label_comparison_contract_ready"] is True
        and freeze["historical_metric_preregistration_ready"] is True
        and freeze["historical_metric_registry_ready"] is True
        and freeze["metric_readiness_ready"] is True
        and freeze["preregistered_metric_count"] > 0
        and freeze["label_runtime_usage_prohibited"] is True
    ...
    return {
        "phase": "21",
        "metric_preregistration_freeze_ready": ready,
        "freeze_id": freeze["freeze_id"],
        "parent_freeze_id": freeze["parent_freeze_id"],
        "freeze_type": freeze["freeze_type"],
        "freeze_manifest_hash": freeze_hash,
        "alpha17_freeze_hash_valid": not missing,
        "freeze_hash_valid": not missing,
        "alpha16_parent_preserved": alpha16_parent_preserved,
        "parent_freeze_present": PARENT_ALPHA16_FREEZE_PATH.exists(),
        "qa12_freeze_unchanged": parent["qa12_freeze_unchanged"],
        "missing_file_count": len(missing),
        "hash_mismatch_count": 0,
        "secret_count": len(secret),
        "production_file_count": len(production),
        "historical_label_comparison_contract_ready": freeze[
            "historical_label_comparison_contract_ready"
        ],
        "historical_metric_preregistration_ready": freeze[
            "historical_metric_preregistration_ready"
        ],
        "historical_metric_registry_ready": freeze[
            "historical_metric_registry_ready"
        ],
        "metric_readiness_ready": freeze["metric_readiness_ready"],
        "preregistered_metric_count": freeze["preregistered_metric_count"],
        "label_runtime_usage_prohibited": freeze["label_runtime_usage_prohibited"],
