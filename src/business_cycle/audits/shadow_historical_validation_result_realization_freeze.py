"""Phase 36 alpha32 historical validation result realization freeze."""
...
DEFAULT_ALPHA32_FREEZE_PATH = Path(
    "specs/audits/book_faithful_shadow_v2_alpha32_historical_validation_result_realization_freeze.yaml"
)
PARENT_ALPHA31_FREEZE_PATH = Path(
    "specs/audits/book_faithful_shadow_v2_alpha31_historical_comparability_realization_freeze.yaml"
)


@lru_cache(maxsize=1)
def summarize_shadow_historical_validation_result_realization_freeze(
    path: str | Path = DEFAULT_ALPHA32_FREEZE_PATH,
) -> dict[str, Any]:
    freeze = yaml.safe_load(Path(path).read_text(encoding="utf-8"))[
        "book_faithful_shadow_v2_alpha32_historical_validation_result_realization_freeze"
    ]
    ...
    parent = summarize_shadow_historical_comparability_realization_freeze()
    audit = summarize_phase36_historical_validation_result_realization()
    alpha31_parent_preserved = (
        PARENT_ALPHA31_FREEZE_PATH.exists()
        and parent["freeze_id"] == freeze["parent_freeze_id"]
        and parent["historical_comparability_realization_freeze_ready"] is True
    )
    ready = (
        not missing
        and not secret
        and not production
        and alpha31_parent_preserved
        and parent["qa12_freeze_unchanged"] is True
        and audit["result"] == "passed"
        and freeze["historical_validation_result_runtime_ready"] is True
        and freeze["recession_recovery_comparability_unblock_ready"] is True
        and freeze["post_validation_result_rerun_ready"] is True
        and freeze["attempted_fix_iteration_count"] >= 2
        and freeze["scenario_count"] == 5
        and freeze["pre_comparable_scenario_count"] == 2
        and freeze["post_comparable_scenario_count"] >= 2
        and freeze["comparable_scenario_count"]
        == freeze["post_comparable_scenario_count"]
        and freeze["historical_validation_result_artifact_count"] > 0
        and freeze["safe_fixable_recession_recovery_gap_count"] == 0
        and freeze["unresolved_safe_fixable_recession_recovery_gap_count"] == 0
        and freeze[
    ...
    return {
        "phase": "36",
        "historical_validation_result_realization_freeze_ready": ready,
        "freeze_id": freeze["freeze_id"],
        "parent_freeze_id": freeze["parent_freeze_id"],
        "freeze_type": freeze["freeze_type"],
        "freeze_manifest_hash": freeze_hash,
        "alpha32_freeze_hash_valid": not missing,
        "freeze_hash_valid": not missing,
        "alpha31_parent_preserved": alpha31_parent_preserved,
        "parent_freeze_present": PARENT_ALPHA31_FREEZE_PATH.exists(),
