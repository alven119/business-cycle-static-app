"""Phase 18 historical validation input readiness closure."""
...
DEFAULT_PHASE18_CLOSURE_PATH = Path(
    "specs/audits/phase18_historical_input_readiness_closure.yaml"
)
...
def summarize_phase18_historical_input_readiness_closure(
    path: str | Path = DEFAULT_PHASE18_CLOSURE_PATH,
) -> dict[str, Any]:
    expected = _load_expected(path)
    readiness = summarize_historical_validation_input_readiness()
    freeze = summarize_shadow_historical_input_readiness_freeze()
    phase17 = summarize_phase17_historical_manifest_closure()
    ...
    summary = {
        ...
        "deferred_capability_gaps": [
            ...
            "decision_runtime_not_executed_on_scenarios",
            "holdout_not_registered",
        ],
        "semantic_drift_count": 0,
        "historical_validation_input_readiness_contract_ready": readiness[
            "historical_validation_input_readiness_contract_ready"
        ],
        "scenario_input_requirements_ready": readiness[
            "scenario_input_requirements_ready"
        ],
        "input_readiness_registry_ready": readiness[
            "input_readiness_registry_ready"
        ],
        "point_in_time_input_availability_ready": readiness[
            "point_in_time_input_availability_ready"
        ],
        "scenario_count": readiness["scenario_count"],
        "scenario_with_complete_input_contract_count": readiness[
        ...
        "alpha14_freeze_hash_valid": freeze["alpha14_freeze_hash_valid"],
        "alpha13_parent_preserved": freeze["alpha13_parent_preserved"],
        ...
        "development_next_phase": 19,
        "prospective_track_next_action": PROSPECTIVE_NEXT_ACTION,
        "phase18_closure_status": CLOSURE_STATUS,
        "project_definition_of_done_progress": (
            "phase18_audits_historical_validation_inputs_without_model_execution"
        ),
        "readiness": readiness,
        "freeze": freeze,
        "phase17_closure": phase17,
        "qa12_closure": qa12,
        "leakage": leakage,
    }
...
def _passed(summary: dict[str, Any], expected: dict[str, Any]) -> bool:
    ...
    return (
        ...
        and summary["freeze"]["historical_input_readiness_freeze_ready"] is True
        ...
        and summary["readiness"]["forbidden_output_field_count"] == 0
        and summary["phase17_closure"]["result"] == "passed"
...
def _load_expected(path: str | Path) -> dict[str, Any]:
    return yaml.safe_load(Path(path).read_text(encoding="utf-8"))[
        "phase18_historical_input_readiness_closure"
    ]["expected"]
