"""Phase 36R alpha33 recession/recovery evidence completion freeze."""
...
def summarize_shadow_recession_recovery_evidence_completion_freeze(
    ...
    parent = summarize_shadow_historical_validation_result_realization_freeze()
    audit = summarize_phase36r_recession_recovery_evidence_completion()
    alpha32_parent_preserved = (
        PARENT_ALPHA32_FREEZE_PATH.exists()
        and parent["freeze_id"] == freeze["parent_freeze_id"]
        and parent["historical_validation_result_realization_freeze_ready"] is True
    )
    ready = (
        not missing
        and not secret
        and not production
        and alpha32_parent_preserved
        and parent["qa12_freeze_unchanged"] is True
        and audit["result"] == "passed"
        and freeze["recession_recovery_evidence_completion_runtime_ready"] is True
        and freeze["post_evidence_completion_validation_rerun_ready"] is True
        and freeze["attempted_fix_iteration_count"] >= 2
        and freeze["scenario_count"] == 5
        and freeze["target_recession_recovery_scenario_count"] == 3
        ...
        and freeze["evidence_completion_false_positive_count"] == 0
        and freeze["evidence_rule_semantics_modified_count"] == 0
    ...
    return {
        "phase": "36R",
        "recession_recovery_evidence_completion_freeze_ready": ready,
        "freeze_id": freeze["freeze_id"],
        "parent_freeze_id": freeze["parent_freeze_id"],
        "freeze_type": freeze["freeze_type"],
        "freeze_manifest_hash": freeze_hash,
        "alpha33_freeze_hash_valid": not missing,
        "freeze_hash_valid": not missing,
        "alpha32_parent_preserved": alpha32_parent_preserved,
        "parent_freeze_present": PARENT_ALPHA32_FREEZE_PATH.exists(),
