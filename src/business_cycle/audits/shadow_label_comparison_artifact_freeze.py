"""Phase 22 alpha18 label-comparison artifact freeze validation."""
...
DEFAULT_ALPHA18_FREEZE_PATH = Path(
    "specs/audits/book_faithful_shadow_v2_alpha18_label_comparison_artifact_freeze.yaml"
)
PARENT_ALPHA17_FREEZE_PATH = Path(
    "specs/audits/book_faithful_shadow_v2_alpha17_metric_preregistration_freeze.yaml"
)


def summarize_shadow_label_comparison_artifact_freeze(
    path: str | Path = DEFAULT_ALPHA18_FREEZE_PATH,
) -> dict[str, Any]:
    freeze = yaml.safe_load(Path(path).read_text(encoding="utf-8"))[
        "book_faithful_shadow_v2_alpha18_label_comparison_artifact_freeze"
    ]
    ...
    parent = summarize_shadow_metric_preregistration_freeze()
    registry = summarize_historical_label_comparison_artifacts()
    alpha17_parent_preserved = (
        PARENT_ALPHA17_FREEZE_PATH.exists()
        and parent["freeze_id"] == freeze["parent_freeze_id"]
        and parent["metric_preregistration_freeze_ready"] is True
    )
    ready = (
        not missing
        and not secret
        and not production
        and alpha17_parent_preserved
        and parent["qa12_freeze_unchanged"] is True
        and registry["label_comparison_artifact_registry_ready"] is True
        and freeze["label_comparison_artifact_contract_ready"] is True
        and freeze["label_comparison_artifact_generator_ready"] is True
        and freeze["label_joiner_ready"] is True
    ...
    return {
        ...
        "prohibited_artifact_field_count": freeze[
