"""Phase 26 alpha22 predicted-label mapping contract freeze validation."""
...
DEFAULT_ALPHA22_FREEZE_PATH = Path(
    "specs/audits/book_faithful_shadow_v2_alpha22_predicted_label_mapping_contract_freeze.yaml"
)
PARENT_ALPHA21_FREEZE_PATH = Path(
    "specs/audits/book_faithful_shadow_v2_alpha21_research_decision_output_freeze.yaml"
)


def summarize_shadow_predicted_label_mapping_contract_freeze(
    path: str | Path = DEFAULT_ALPHA22_FREEZE_PATH,
) -> dict[str, Any]:
    freeze = yaml.safe_load(Path(path).read_text(encoding="utf-8"))[
        "book_faithful_shadow_v2_alpha22_predicted_label_mapping_contract_freeze"
    ]
    ...
    parent = summarize_shadow_research_decision_output_freeze()
    readiness = summarize_offline_predicted_label_mapping_readiness()
    alpha21_parent_preserved = (
        PARENT_ALPHA21_FREEZE_PATH.exists()
        and parent["freeze_id"] == freeze["parent_freeze_id"]
        and parent["research_decision_output_freeze_ready"] is True
    )
    ready = (
        not missing
        and not secret
        and not production
        and alpha21_parent_preserved
        and parent["qa12_freeze_unchanged"] is True
        and readiness["predicted_label_mapping_readiness_ready"] is True
        and freeze["predicted_label_mapping_contract_ready"] is True
        and freeze["predicted_label_mapping_readiness_ready"] is True
        and freeze["research_decision_state_taxonomy_ready"] is True
        and freeze["offline_predicted_label_taxonomy_ready"] is True
        and freeze["mapping_rule_count"] > 0
        and freeze["predicted_label_output_count"] == 0
        and freeze["predicted_label_artifact_count"] == 0
    ...
    return {
        "phase": "26",
        "predicted_label_mapping_freeze_ready": ready,
        "freeze_id": freeze["freeze_id"],
        "parent_freeze_id": freeze["parent_freeze_id"],
        "freeze_type": freeze["freeze_type"],
        "freeze_manifest_hash": freeze_hash,
        "alpha22_freeze_hash_valid": not missing,
        "freeze_hash_valid": not missing,
        "alpha21_parent_preserved": alpha21_parent_preserved,
        "parent_freeze_present": PARENT_ALPHA21_FREEZE_PATH.exists(),
        "qa12_freeze_unchanged": parent["qa12_freeze_unchanged"],
        "missing_file_count": len(missing),
        "hash_mismatch_count": 0,
        "secret_count": len(secret),
        "production_file_count": len(production),
        "predicted_label_mapping_contract_ready": freeze[
            "predicted_label_mapping_contract_ready"
        ],
        "predicted_label_mapping_readiness_ready": freeze[
            "predicted_label_mapping_readiness_ready"
        ],
        "research_decision_state_taxonomy_ready": freeze[
            "research_decision_state_taxonomy_ready"
        ],
        "offline_predicted_label_taxonomy_ready": freeze[
            "offline_predicted_label_taxonomy_ready"
        ],
        "mapping_rule_count": freeze["mapping_rule_count"],
