"""Phase 27 predicted-label artifact closure."""
...
DEFAULT_PHASE27_CLOSURE_PATH = Path(
    "specs/audits/phase27_predicted_label_artifact_closure.yaml"
)
...
def summarize_phase27_predicted_label_artifact_closure(
    path: str | Path = DEFAULT_PHASE27_CLOSURE_PATH,
) -> dict[str, Any]:
    expected = _load_expected(path)
    readiness = summarize_offline_predicted_label_artifact_readiness()
    freeze = summarize_shadow_predicted_label_artifact_freeze()
    phase26 = summarize_phase26_predicted_label_mapping_contract_closure()
    ...
    summary = {
        ...
        "offline_predicted_label_artifact_contract_ready": readiness[
            "offline_predicted_label_artifact_contract_ready"
        ],
        "offline_predicted_label_artifact_generator_ready": readiness[
            "offline_predicted_label_artifact_generator_ready"
        ],
        "offline_predicted_label_artifact_readiness_ready": readiness[
            "offline_predicted_label_artifact_readiness_ready"
        ],
        "scenario_count": readiness["scenario_count"],
        "research_decision_output_count": readiness[
        ...
        "predicted_label_provenance_complete_count": readiness[
        ...
        "alpha23_freeze_hash_valid": freeze["alpha23_freeze_hash_valid"],
        "alpha22_parent_preserved": freeze["alpha22_parent_preserved"],
        ...
        "development_next_phase": 28,
        "prospective_track_next_action": PROSPECTIVE_NEXT_ACTION,
        "phase27_closure_status": CLOSURE_STATUS,
        "project_definition_of_done_progress": (
            "phase27_materializes_offline_predicted_labels_without_comparison"
        ),
        "readiness": readiness,
        "freeze": freeze,
        "phase26_closure": phase26,
        "leakage": leakage,
    }
...
def _passed(summary: dict[str, Any], expected: dict[str, Any]) -> bool:
    ...
    return (
        ...
        and summary["readiness"]["offline_predicted_label_artifact_readiness_ready"]
        is True
        and summary["freeze"]["predicted_label_artifact_freeze_ready"] is True
        ...
        and summary["phase26_closure"]["result"] == "passed"
        and summary["phase26_closure"]["prospective_registry_record_count"] == 0
        and summary["phase26_closure"]["real_registry_write_attempt_count"] == 0
    )
...
def _load_expected(path: str | Path) -> dict[str, Any]:
    return yaml.safe_load(Path(path).read_text(encoding="utf-8"))[
        "phase27_predicted_label_artifact_closure"
    ]["expected"]
