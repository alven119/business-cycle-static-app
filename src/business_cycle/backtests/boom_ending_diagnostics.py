"""Diagnostics for experimental boom-ending candidate indicators."""
...
from business_cycle.backtests.boom_ending_candidates import score_boom_ending_candidate_indicators
...
class BoomEndingDiagnosticsError(ValueError):
    """Raised when boom-ending diagnostics cannot be built."""


@dataclass(frozen=True)
class BoomEndingDiagnosticPoint:
    """One scenario/as-of boom-ending diagnostic point."""
...
@dataclass(frozen=True)
class BoomEndingDiagnosticWindows:
    """Boom-ending diagnostic window spec."""

    version: int
    status: str
    data_mode: str
    caveats_zh: list[str]
    points: list[BoomEndingDiagnosticPoint]


def load_boom_ending_diagnostic_windows(
    path: str | Path = DEFAULT_WINDOWS_PATH,
) -> BoomEndingDiagnosticWindows:
    """Load and validate boom-ending diagnostic windows."""

    payload = _load_yaml_mapping(path)
    raw = payload.get("boom_ending_diagnostic_windows")
    if not isinstance(raw, dict):
        raise BoomEndingDiagnosticsError(
            "boom_ending_diagnostic_windows YAML must contain a mapping"
        )
    caveats = _non_empty_str_list(raw.get("caveats_zh"), "caveats_zh")
    if not any("修訂後歷史資料" in caveat for caveat in caveats):
        raise BoomEndingDiagnosticsError("caveats_zh must include revised data caveat")
    if not any("不構成投資建議" in caveat for caveat in caveats):
        raise BoomEndingDiagnosticsError("caveats_zh must include no-investment-advice caveat")
    points = _points_from_scenarios(raw.get("scenarios"))
    return BoomEndingDiagnosticWindows(
...
def is_boom_ending_high_signal(score: dict[str, Any]) -> bool:
    """Return whether a candidate score is a high boom-ending signal."""

    return _num(score.get("score")) >= HIGH_SIGNAL_SCORE and _num(score.get("confidence")) >= HIGH_SIGNAL_CONFIDENCE


def is_boom_ending_strong_signal(score: dict[str, Any]) -> bool:
    """Return whether a candidate score is a strong high-confidence boom-ending signal."""

    return _num(score.get("score")) >= STRONG_SIGNAL_SCORE and _num(score.get("confidence")) >= STRONG_SIGNAL_CONFIDENCE


def build_boom_ending_point_summary(
    scores: list[dict[str, Any]],
    failures: list[dict[str, Any]],
    groups_by_indicator: dict[str, str],
) -> dict[str, Any]:
    """Build deterministic point-level summary from boom-ending candidate scores."""

    high_scores = [score for score in scores if is_boom_ending_high_signal(score)]
    strong_scores = [score for score in scores if is_boom_ending_strong_signal(score)]
    high_groups = {
        groups_by_indicator.get(str(score.get("indicator_id")))
        for score in high_scores
        if groups_by_indicator.get(str(score.get("indicator_id")))
    }
    weighted_score = _weighted_boom_ending_score(scores)
    return {
        ...
        "boom_ending_status": _boom_ending_status(
            broad_group_count=len(high_groups),
            high_signal_count=len(high_scores),
            high_confidence_high_signal_count=len(strong_scores),
        ),
...
def build_boom_ending_group_summary(
    scores: list[dict[str, Any]],
    groups: dict[str, list[str]],
) -> list[dict[str, Any]]:
    """Build group-level boom-ending candidate score summaries."""
    ...
    for group_id, indicator_ids in groups.items():
        ...
        high_signal_count = sum(1 for score in group_scores if is_boom_ending_high_signal(score))
...
def build_boom_ending_diagnostics(
    *,
    ...
    """Build boom-ending diagnostics across configured scenario points."""
    ...
    scorer = score_func or score_boom_ending_candidate_indicators
    ...
    for point in windows.points:
        ...
        point_summary = build_boom_ending_point_summary(scores, failures, groups_by_indicator)
...
def write_boom_ending_diagnostics(output_path: str | Path, diagnostics: dict[str, Any]) -> Path:
    """Write boom-ending diagnostics JSON."""
...
def load_experimental_indicator_groups(path: str | Path = DEFAULT_GROUPS_PATH) -> dict[str, list[str]]:
    ...
    if not isinstance(raw_groups, dict):
        raise BoomEndingDiagnosticsError("experimental_indicator_groups YAML must contain a mapping")
    groups: dict[str, list[str]] = {}
...
def _points_from_scenarios(raw_scenarios: Any) -> list[BoomEndingDiagnosticPoint]:
    if not isinstance(raw_scenarios, dict) or not raw_scenarios:
        raise BoomEndingDiagnosticsError("scenarios must be a non-empty mapping")
    points: list[BoomEndingDiagnosticPoint] = []
    for scenario_id, scenario in raw_scenarios.items():
        if not isinstance(scenario, dict):
            raise BoomEndingDiagnosticsError("scenario entries must be mappings")
        display_name_zh = str(scenario.get("display_name_zh") or "")
        raw_points = scenario.get("diagnostic_points")
        if not isinstance(raw_points, list) or not raw_points:
            raise BoomEndingDiagnosticsError(f"scenario {scenario_id} diagnostic_points must be non-empty")
        for point in raw_points:
            if not isinstance(point, dict):
                raise BoomEndingDiagnosticsError(
                    f"scenario {scenario_id} diagnostic_points entries must be mappings"
                )
            points.append(
                BoomEndingDiagnosticPoint(
    ...
    if any(not point.as_of or not point.label for point in points):
        raise BoomEndingDiagnosticsError("diagnostic points must include as_of and label")
    return points
...
def _build_comparisons(points: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    early = by_label.get("gfc_yield_curve_warning")
    confirmed = by_label.get("gfc_confirmed_recession")
    if early and confirmed:
        early_summary = early["candidate_summary"]
        confirmed_summary = confirmed["candidate_summary"]
        comparisons["gfc_2006_vs_2008"] = {
            "early_warning_as_of": early["as_of"],
            "confirmed_recession_as_of": confirmed["as_of"],
            "score_delta": round(
                confirmed_summary["weighted_boom_ending_score"]
                - early_summary["weighted_boom_ending_score"],
                4,
            ),
            "group_breadth_delta": confirmed_summary["broad_group_count"] - early_summary["broad_group_count"],
            "interpretation_zh": "比較 GFC 早期殖利率曲線警訊與 confirmed recession 時點的候選指標廣度與分數差異。",
        }
    covid_2019 = by_label.get("covid_2019_false_recession_context")
    covid_2020 = by_label.get("covid_shock_recession")
    if covid_2019 and covid_2020:
        base = covid_2019["candidate_summary"]
        shock = covid_2020["candidate_summary"]
        comparisons["covid_2019_vs_2020"] = {
            "late_cycle_context_as_of": covid_2019["as_of"],
            "shock_recession_as_of": covid_2020["as_of"],
            "score_delta": round(
                shock["weighted_boom_ending_score"] - base["weighted_boom_ending_score"],
                4,
            ),
            "group_breadth_delta": shock["broad_group_count"] - base["broad_group_count"],
            "interpretation_zh": "比較 COVID 2019 late-cycle context 與 2020 外生衝擊期間；boom ending 指標不應被視為 confirmed recession rule。",
        }
    return comparisons
...
def _aggregate(points: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "candidate_indicators_useful_for_early_warning": useful,
...
def _useful_candidate_indicators(points: list[dict[str, Any]]) -> list[str]:
    early_warning_labels = {
        "dotcom_market_peak_area",
        "gfc_yield_curve_warning",
        "gfc_recession_window_start",
        "late_cycle_2018_warning",
    }
    useful: list[str] = []
    for point in points:
        if point["label"] not in early_warning_labels:
            continue
        for score in point.get("top_candidate_scores", []):
            indicator_id = str(score.get("indicator_id"))
            if is_boom_ending_high_signal(score) and indicator_id not in useful:
...
def _indicators_requiring_refinement(points: list[dict[str, Any]], useful: list[str]) -> list[str]:
    flagged: list[str] = []
    non_recession_context_labels = {
        "covid_2019_false_recession_context",
        "euro_debt_slowdown_warning",
    }
    for point in points:
        if point["label"] not in non_recession_context_labels:
            continue
        for score in point.get("top_candidate_scores", []):
            indicator_id = str(score.get("indicator_id"))
            if (
                is_boom_ending_strong_signal(score)
                and indicator_id not in useful
                and indicator_id not in flagged
            ):
                flagged.append(indicator_id)
    return flagged


def _boom_ending_status(
    *,
...
def _weighted_boom_ending_score(scores: list[dict[str, Any]]) -> float:
...
def _diagnostic_notes(summary: dict[str, Any], failures: list[dict[str, Any]]) -> list[str]:
    notes: list[str] = ["此診斷只解釋 boom ending candidate indicators，不會修改正式模型判斷。"]
    if failures:
        notes.append("部分 candidate indicators 缺少本機 cache 或 scoring failure，需先補齊資料再解讀。")
    if summary["boom_ending_status"] == "strong":
        notes.append("候選指標呈現廣泛且高信心的榮景期結束或 late-cycle pressure。")
    elif summary["boom_ending_status"] == "watch":
        notes.append("候選指標呈現多群組 late-cycle pressure，適合作為觀察訊號而非正式階段結論。")
    return notes
...
def _load_yaml_mapping(path: str | Path) -> dict[str, Any]:
    ...
    if not yaml_path.exists():
        raise BoomEndingDiagnosticsError(f"Boom ending diagnostics file does not exist: {yaml_path}")
    try:
        payload = yaml.safe_load(yaml_path.read_text(encoding="utf-8"))
    except yaml.YAMLError as exc:
        raise BoomEndingDiagnosticsError(f"Invalid YAML in {yaml_path}: {exc}") from exc
    if not isinstance(payload, dict):
        raise BoomEndingDiagnosticsError("YAML must be a mapping")
    return payload


def _non_empty_str_list(value: Any, field: str) -> list[str]:
    if not isinstance(value, list) or not value:
        raise BoomEndingDiagnosticsError(f"{field} must be a non-empty list")
    items = [str(item) for item in value if str(item)]
    if len(items) != len(value):
        raise BoomEndingDiagnosticsError(f"{field} entries must be non-empty")
    return items
