...
def test_phase31_validation_blockage_remediation_closure_passes() -> None:
    ...
    assert summary["development_next_phase"] == 32
    assert summary["phase31_closure_status"] == (
        "closed_validation_blockage_remediation_reviewed_safe_fixes_only_no_performance"
    )


def test_show_phase31_validation_blockage_remediation_closure_script() -> None:
    result = subprocess.run(
        [sys.executable, "scripts/show_phase31_validation_blockage_remediation_closure.py"],
        check=True,
        capture_output=True,
        text=True,
    )

    assert "phase=31" in result.stdout
