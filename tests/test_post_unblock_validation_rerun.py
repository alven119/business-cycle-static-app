...
from business_cycle.validation.post_unblock_validation_rerun import (
    build_post_unblock_validation_rerun,
    write_post_unblock_validation_rerun,
)


def test_post_unblock_validation_rerun_is_ready() -> None:
    run = build_post_unblock_validation_rerun()

    assert run["post_unblock_validation_rerun_ready"] is True
...
def test_post_unblock_validation_rerun_writes_bundle_under_tmp(
    tmp_path: Path,
) -> None:
    result = write_post_unblock_validation_rerun(
        build_post_unblock_validation_rerun(),
        output_dir=tmp_path / "phase34_post_unblock",
    )

    assert result["post_unblock_validation_rerun_written"] is True
...
def test_post_unblock_validation_rerun_rejects_repo_output() -> None:
    with pytest.raises(ValueError):
        write_post_unblock_validation_rerun(
            build_post_unblock_validation_rerun(),
            output_dir="data/backtests/phase34_post_unblock",
        )


def test_run_post_unblock_validation_rerun_script(tmp_path: Path) -> None:
    output_dir = tmp_path / "phase34_post_unblock"
    result = subprocess.run(
        [
            sys.executable,
            "scripts/run_post_unblock_validation_rerun.py",
            "--output-dir",
            str(output_dir),
        ],
        check=True,
    ...
    assert "post_unblock_validation_rerun_ready=true" in result.stdout
    assert "post_resolution_blocked_scenario_count=0" in result.stdout
    assert (output_dir / "phase34_research_artifact_explorer.html").is_file()
