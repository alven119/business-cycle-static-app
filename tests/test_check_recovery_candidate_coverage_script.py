...
def test_check_recovery_candidate_coverage_outputs_summary() -> None:
    completed = run_script()

    assert completed.returncode == 0, completed.stderr
    assert "candidate_indicator_count=10" in completed.stdout
...
def test_check_recovery_candidate_coverage_counts_fake_cache(tmp_path: Path) -> None:
    cache_dir = tmp_path / "fred"
    cache_dir.mkdir()
    write_csv(cache_dir / "ICSA.csv")
    ...
    assert "cached_series=BAA,ICSA" in completed.stdout


def test_check_recovery_candidate_coverage_missing_cache_does_not_crash(tmp_path: Path) -> None:
...
def run_script(*args: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [sys.executable, "scripts/check_recovery_candidate_coverage.py", *args],
        cwd=project_root,
        env={"PATH": ""},
        check=False,
        capture_output=True,
