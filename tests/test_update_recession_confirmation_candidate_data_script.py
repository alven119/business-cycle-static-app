...
import scripts.update_recession_confirmation_candidate_data as update_script
...
def test_update_candidate_data_dry_run_succeeds_without_api_key(tmp_path: Path) -> None:
    completed = run_script("--dry-run", "--raw-dir", str(tmp_path / "raw"))

    assert completed.returncode == 0, completed.stderr
    assert "required_series_count=11" in completed.stdout
    assert "downloaded_series_count=0" in completed.stdout
    assert "FRED_API_KEY" not in completed.stdout


def test_update_candidate_data_no_api_succeeds_without_api_key(tmp_path: Path) -> None:
...
def test_update_candidate_data_missing_api_key_fails(
    tmp_path: Path,
...
def test_update_candidate_data_main_can_use_fake_provider(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
...
def run_script(*args: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [sys.executable, "scripts/update_recession_confirmation_candidate_data.py", *args],
        cwd=project_root,
        env={"PATH": ""},
        check=False,
        capture_output=True,
