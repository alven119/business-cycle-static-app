...
Production v1 remains a preserved baseline for historical compatibility. It is
not the mature product doctrine described by
...
and snapshot after the dashboard path moves to the doctrine-aligned NAS dynamic
service.
...
| `.github/workflows/pages.yml` | retired GitHub Pages deployment workflow | Must remain absent unless a future explicit migration gate reopens Pages |
...
- dashboard and deployment-target changes

The Pages deployment path is retired. Legacy v1 scoring, resolver, pipeline,
and snapshot artifacts remain quarantined as compatibility baselines and must
not be promoted into the mature product answer.
