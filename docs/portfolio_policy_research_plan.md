...
Phase 9A8 新增 real backtest execution readiness closure。

```bash
python scripts/show_real_backtest_execution_readiness_closure.py
```

Closure 彙整 Phase 9A–9A7 的 contract stack，確認 engine、result output、metric registry、output location、caveat、safety validator、fixtures 與 writer contracts 已具備進入 Phase 9B controlled prototype 的前置條件。9A8 可宣告 9B entry readiness，但本階段仍不得執行 backtest、計算績效、產生 result、建立 output directory 或寫入 `data/backtests` / `public`。
