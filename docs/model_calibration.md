...
Phase 8E 建立 portfolio backtest dry-run engine contract。

```bash
python scripts/show_portfolio_backtest_dry_run_contract.py
```

Dry-run contract 只允許 structural validation 與 stdout summary。它禁止績效計算、portfolio weights、allocation、target weight、buy/sell signal、`data/backtests` output 與 `public` output。Dry-run summary 不是 backtest result，也不構成投資建議。

## Phase 8F Portfolio Backtest Dry-Run Fixtures

Phase 8F 建立 dry-run output fixtures 與 validator。
...
Phase 9A 建立 future real backtest engine contract。

```bash
python scripts/show_real_backtest_engine_contract.py
```

Engine contract 定義 future engine scope、required input contracts、future dependency contracts、engine stages、prohibited outputs、prohibited auto-write locations 與 execution 前必須通過的 safety guards。本階段仍不實作 engine runtime、不執行回測、不計算績效、不產生 result、不產生 allocation、不輸出 trade signal，也不寫入 `data/backtests` 或 `public`。
...
Phase 9A4 建立 backtest result caveat policy。

```bash
python scripts/show_backtest_result_caveat_policy.py
```

Caveat policy 定義 future result 必須包含的 global caveats、contextual caveats、display requirements、prohibited text patterns 與 future validation rules。它要求 caveats 必須在任何 metric value 前可見，並禁止目前建議、買進訊號、賣出訊號、target weight 與 investment advice 等文字。
...
Phase 9A6 建立 safety validator fixtures。

```bash
python scripts/validate_backtest_result_safety_validator_fixtures.py
```

Fixtures 包含合法 schema/safety samples，以及故意違規的 live allocation、target weight、buy/sell signal、current recommendation、public dashboard output、phase override、prohibited text、caveat 缺失、caveat 不可見與 output location violation samples。Fixture-only validator 只驗證這些樣本是否被接受或拒絕，不等於 real result validator runtime。
...
Phase 9B 建立 controlled synthetic in-memory calculation harness。

```bash
python scripts/run_controlled_real_backtest_prototype.py
```

Prototype 只使用 controlled synthetic fixtures，在 memory 中建立 value path、套用 backtest-only policy schedule，並計算受控 arithmetic metrics count。CLI 只輸出 summary flags 與 counts，不輸出完整 metric table。

本階段仍不寫 result file、不建立 output directory、不寫入 `data/backtests` 或 `public`、不接 dashboard、不產生 allocation、不產生 trade signal。QA0 審核完成前暫停 9B1；9B 不得宣稱已驗證書籍策略、歷史績效、景氣模型或 point-in-time 可交易性。
