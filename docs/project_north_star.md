...
本專案的最終目標，是建立一套可長期使用、可追溯、可解釋、具 point-in-time 意識的總體經濟研究網頁系統。系統應定期取得官方總體資料，維護 declared current cycle state，依照 `衰退 -> 復甦 -> 成長 -> 榮景 -> 衰退` 的合法順序監測下一個 transition，解釋所有 observation、phase evidence、watch、confirmation、abstention 的資料與規則來源，並提供安全隔離的歷史重播、回測與 portfolio policy research。

系統不得將研究結果自動轉化為即時買賣建議、目標權重或交易訊號。任何正式輸出均必須揭露資料模式、完整度、模型版本、驗證狀態、限制與用途。

最終系統必須能回答：

1. 目前 declared current cycle phase 是什麼，何時開始，已持續多久？
2. 合法下一階段是什麼，哪些 transition evidence 支持、反對或尚未具備？
3. 是否正在接近合法階段轉折，或只是 observation／watch？
4. 這是 observation、phase evidence、watch、confirmation，還是 ordered state-machine phase decision？
5. 使用的是當時可取得的資料，還是後來修訂資料？
6. state machine 或 evidence profile 為何維持 declared state、為何提示 transition readiness，或為何 abstain？
...
系統定期取得 FRED、ALFRED、BEA、BLS、DOL、Census、EIA 等官方資料，計算 level、YoY、moving average、slope、percentile、z-score 等轉換，並以書籍 major groups 與經過治理的 modern methodology 形成 phase evidence profile 與 transition readiness profile。成熟輸出必須以 declared current phase、declared phase age、legal next phase、legal transition candidate、ordered state-machine decision／abstention、evidence completeness、confidence／uncertainty、major-group readiness、由證據結構推導的前段／中段／後段位置，以及 abstention reason 為核心。Phase evidence profile 是 explanation input，不是 standalone selector；phase score 只能作為 legacy／modern diagnostic，不得作為 product answer；weighted scoring 若存在，必須標記為 modern methodology，不得宣稱為書中明訂權重。
...
必須支援 point-in-time data mode、release／revision semantics、strict vs revised 分離、missing evidence abstention、incomplete phase evidence／transition readiness 不進 formal ordered state-machine transition，以及 historical strict readiness 與 forward capture readiness 分離。
...
| M1 | 指標轉換與 evidence 基礎 | production v1 legacy scoring 已具備；book-faithful shadow roles 多數仍為 observation/raw-only 或 phase-evidence input |
| M2 | Ordered cycle state-machine model | production v1 有 legacy baseline；book-faithful v2 declared state registry、legal transition monitor 與 formal ordered decision 尚未完成 |
| M3 | 書籍指標對齊 | 40 roles、24 major groups 已盤點；來源、規則與 evidence 持續補齊 |
| M4 | Historical replay／diagnostics／calibration | diagnostics 已有；transition timing replay、policy replay、calibration、validation 與 holdout 尚未完成 |
...
| M10 | Model calibration | 尚未正式開始；既有 scenarios 僅可作 development calibration，不得用 static label answer 反推 transition rule 或 threshold |
...
首頁必須立即回答 declared current phase、declared phase age、legal next phase、transition readiness、判斷完整度、本月變化與資料可信度。核心卡片包含 declared current phase、legal transition candidate／abstention、evidence completeness、confidence／uncertainty、major groups ready／partial／missing、late-cycle／recession／recovery watches、data health、model／freeze／validation status。不得只顯示單一分數或 winner；若證據不完整，應顯示 partial 或 abstained。

### 8.2 景氣階段分析頁

可切換復甦、成長、榮景、衰退，查看 phase evidence profile、major groups、supportive／contradictory／mixed／unavailable、observation-only／phase-evidence／transition-evidence、declared state relation、legal transition relation 與 abstention reason。此頁解釋 evidence，不獨立選出 current phase；復甦至少依就業、消費、投資、進出口四組呈現；其他 phase 依各自書籍 major groups 呈現。

### 8.3 景氣時間軸

顯示四階段 evidence timeline、legacy diagnostic score timeline、declared phase 區間、legal transition candidate、transition watch／confirmation、abstention、data gaps、shock overlay、model version change points，並支援日期點擊、縮放、拖曳、production／shadow、revised／vintage_as_of 比較。Timeline 必須區分 declared state、transition watch、transition confirmation 與 diagnostic score。
...
可選 Dotcom、GFC、COVID、Euro debt、2018 late cycle 或自訂期間，逐月播放當時可取得資料、role／group evidence、declared state、legal transition candidate、transition watch、transition confirmation、abstention、revised vs point-in-time 差異。在 temporal gate 未通過前，只能標為 diagnostics。
...
顯示 productivity-driven、inflation-driven、mixed／uncertain。此層影響長期 portfolio research，不得混入四階段 ordered transition decision。

### 8.11 Shock Overlay 頁

顯示 shock detected、normal-cycle declared state、emergency watch、temporary shock state、normalization review 與 caveats。不得直接 override formal ordered state-machine phase 或產生 portfolio action。

### 8.12 Production v1 vs Shadow v2 比較頁

比較 declared state／legal transition candidate／decision status、context prior usage、completeness／abstention、book fidelity、model version／validation、差異來源與 attribution。Legacy v1 score outputs must be labeled as legacy baseline diagnostics.
...
只有當下列能力都具備並通過相應 gate，才可視為成熟版本：官方資料來源與 adapters 可維運、book-core roles 與 major groups 有完整 contract、point-in-time 與 revised 模式明確、declared current phase registry 與 ordered state machine 可輸出 legal transition candidate／formal decision／abstention、transition evidence 不與 formal phase 混用、所有輸出可解釋與追溯、historical replay 可用、書中五策略與 portfolio policy 可安全研究、strict backtest 可用且 cash-flow-aware、dashboard 完整接線、prospective registry append-only 且已前瞻驗證、production migration 有 rollback 與版本治理、automation／deployment／source health 可長期維運、任何研究結果都不會自動變成交易建議。
