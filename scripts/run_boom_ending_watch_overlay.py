"""Run full-horizon experimental boom-ending watch overlay."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run experimental boom-ending watch overlay.")
    parser.add_argument("--experiment-id", default="boom_ending_watch_overlay_v1", help="Experiment ID.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.reuse_existing and not args.force and output_path.exists():
            ...
            output_path = write_boom_ending_watch_overlay_report(output_path, report)
    except (BoomEndingWatchOverlayError, json.JSONDecodeError) as exc:
        parser.exit(status=1, message=f"error: {exc}\n")

    summary = report["acceptance_summary"]
    density = report["global_watch_density_summary"]
    ...
    print(f"dotcom_has_pre_recession_watch={str(summary['dotcom_has_pre_recession_watch']).lower()}")
    print(f"gfc_has_pre_recession_watch={str(summary['gfc_has_pre_recession_watch']).lower()}")
    print(f"euro_debt_excessive_watch={str(summary['euro_debt_excessive_watch']).lower()}")
    print(f"late_cycle_2018_excessive_watch={str(summary['late_cycle_2018_excessive_watch']).lower()}")
    print(f"global_watch_density_ratio={density['watch_density_ratio']}")
    print(f"global_strong_density_ratio={density['strong_density_ratio']}")
