"""Score experimental boom-ending candidate indicators from local cache."""
...
DEFAULT_OUTPUT_ROOT = Path("data/backtests/candidate_indicators/boom_ending")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Score experimental boom-ending candidate indicators from local raw cache."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        scores = score_boom_ending_candidate_indicators(
            as_of=args.as_of,
            cache_dir=args.cache_dir,
            spec_path=args.spec,
        )
        output_path = write_boom_ending_candidate_scores(output, scores)
