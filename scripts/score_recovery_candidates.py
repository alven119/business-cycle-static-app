"""Score experimental recovery candidate indicators from local cache."""
...
DEFAULT_OUTPUT_ROOT = Path("data/backtests/candidate_indicators/recovery")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Score experimental recovery candidate indicators from local raw cache."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        scores = score_recovery_candidate_indicators(
            as_of=args.as_of,
            cache_dir=args.cache_dir,
            spec_path=args.spec,
        )
        output_path = write_recovery_candidate_scores(output, scores)
